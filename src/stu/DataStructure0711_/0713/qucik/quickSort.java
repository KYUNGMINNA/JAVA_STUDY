...
public class QuickSort {
	...
	private static void quickSort(int[] a , int leftmost,int rightmost) {
		//leftmost가 rightmost보다 크거나 같다면 정렬할 원소가
	...
	private static int partition(int[] a , int left,int right) {
		...
		while(start<end) {
			...
			/*
			 	end가 start보다 크면서, start 요소가 pivot보다 큰 원소를
			 	찾을 때 까지 start를 하나씩 증가시킵니다.
			 */
			...
			/*
			 - end 가 start보다 크면서 ,end의 요소가 pivot보다 작거나 같은 원소를
			 찾을 때 까지 end를 하나씩 감소시킨다.
			 */
			
			while(start<end && a[--end]>=pivot);
			
			//위의 두 반복문이 멈췄다면 , start와 end가 가리키는 값을 
			//swap 한다
			swap(a,start,end);
		...
		/*
		 while(start<end)의 끝은 , start와 end가 만났다는 것 
		 맨 처음 pivot으로 설정했던 위치의 원소와 end가 가리키는 원소를 서로
		 swap해 줍니다.
		 */
		swap(a, right, end);
		
		//두 요소가 교환되었다면  피벗이었던 요소는 end에 위치하므로 end를 반환
		return end;
