...
<body>
 ...
 <%--
 	...
 	많은 정보가 들어가 있습니다. (요청에 대한 전반적인 정보를 갖고 있다)
 	request 객체는 내장객체 입니다. 직접 생성하지 않아도 
 	요청이 들어올 때 자동으로 객체가 생성되기 때문에 바로 사용하면 된다.
  --%>
  
  URL 주소:<%=request.getRequestURL() %><br>
  URI 주소 :<%=request.getRequestURI() %><br>
  컨텍스트 루트 :<%=request.getContextPath() %><br> <!--내부 경로 시작주소 -->
