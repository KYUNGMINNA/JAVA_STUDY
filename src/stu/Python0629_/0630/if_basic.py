...
-키웓 if 뒤에 조건식을 적고, 조건식의 결과가 참이면
if 문 실행, 거짓인 경우 else문이 실행됩니다.

-if 문은 단독 사용이 가능하지만, else는 반드시 if와 함께 사용해야 한다.
'''
age=int(input('몇 살 이세요 ?:"'))
...
-타 언어는 중괄호 ({})로 종속 구조를 표현 할 때
파이썬은 중괄호 작성하지 않습니다.

-그 대신 콜론( : )과 들여쓰기로 블록 구조를 표현합니다.

-if문 뿐만이 아니라 ,코드의 종속 구조를 나타내는 모든 문법에서
(반복문,함수 선언 ...)콜론과 들여쓰기를 사용합니다.
파이썬의 권장 들여쓰기 횟수는 스페이스 4회 입니다.
