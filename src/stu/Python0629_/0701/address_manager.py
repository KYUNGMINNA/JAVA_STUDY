...
while True:
    ...
    if menu == 1:
        ...
        if name in namelist:
            ...
            print(f'{name}의 전화번호는 {phonelist[index]}입니다.')
        else:
            print(f'{name}님은 연락처 목록레 없습니다.')
        ...
        if name not in namelist:
            ...
            #del phonelist[index]  # del은  괄호 생략이 가능하다 
            print(f'{name}님 이름과 {p}전화번호를 삭제하였습니다.')
        ...
        for index in range(len(namelist)):
            print(namelist[index]+':'+phonelist[index])
            print('-'*10)
