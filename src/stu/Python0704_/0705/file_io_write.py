...
파일이 존재하지 않는다면 새롭게 생성도 해 줍니다.  :: 기존 파일에 새로 추가하는것 
'''
#파일 입출력은 외부 데이터로 접근하기 때문에 
#예외 처리를 통해 안전하게 작성합니다.
try:
    #파일 저장 기능 (write ,append)
    file_path=r'C:\Users\kyungmin\java_web_vs\python\test\test2.txt'  #파일 명까지 쓰자 

    #파일 입출력을 실행하는 내장 함수 open()
    #반환값으로 파일 입출력을 담당하는 객체가 리턴된다.
    #첫번째 인수로 파일경로를 , 두번째 인수로 모드를 설정 
    f=open(file_path,'a') #모드는 문자열 형태로 
    
   

    text='a모드는 기존 파일만 지정할 수 있는것이 아닌 새파일도 지정이 가능합니다. 새 파일 생성 가능'
    ...
    #반드시 사용 후 그 자원을 해제하여 메모리 누스를 방지합시다.
    f.close()