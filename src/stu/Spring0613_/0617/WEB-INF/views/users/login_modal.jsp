...
<script>
   ...
   $(function() {
      ...
      //입력값 중 하나라도 만족하지 못한다면 회원가입 처리를 막기 위한 논리형 변수 
      ...
      $('#user_id').keyup(function() {
         ...
         if($(this).val() === '') {
            ...
            chk1=false;//사용자가 정상으로 입력했다가, 다르게 입력할 때 오류를 발생할 수 있기 때문에  여기에도 false로 쓴다.
         }
         ...
         //특정 로직의 실행이 끝날 때까지 기다리지 않고 먼저 코드를 실행.(페이지 전환 없이 통신)
            ...
            //ajax함수 안에서 $(this) 가 동작안할수 도 있어 확실히 지목하자
            $.ajax({
               ...
               data: id, //서버로 전송할 데이터 :const id 임
      ...
      $('#password_check').keyup(function(){
    	 	...
    	 	if($(this).val()===''){
    	 		 $(this).css('background', 'pink');
                 $('#pwChk2').html('<b style="font-size: 14px; color: red">[비밀번호 확인은 필수정보입니다!]</b>');
                 chk3=false;
    	 	}
    	 	//비밀번호 확인란 유효성 검사 
      ...
      //사용자가 회원가입 버튼을 눌렀을 때의 이벤트 처리 
      //사용자가 입력하는 4가지 데이터 중 하나라도 문제가 있다면 
      //회원 가입 처리 해주지 않는다.
     ...
     $('#signup-btn').click(function(){
    	 if(chk1 && chk2 && chk3 && chk4){
    		 ...
    		 //여러개의 값을 보낼 때는 객체로 포장해서 전송
    		 //property 이름은 VO객체의 변수명과 동일하게 작성(커맨드 객체 VO를 사용하기 위해서)
    		 const user={
    				 'account':id,
    				 'password':pw,
    				 'name':name	 
    		 };//아직 자바스크리븥 객체 ( JSON 아님!!!!!!!!!!!!!!!!!!)
    		 //객체 같은경우 JSON으로 변환해서 보내야 함!! 값 1개는 그냥 보내도 됌
    		 ...
    		 $.ajax({
				...
				dataType:'text',
				//JS객체를 JSON 문자열로 변환해 주는 메서드  :user로만 보내면 JS객체라오류	
				data:JSON.stringify(user),
				success:function(result){ //서버가 넘겨주는 데이터를 result에 받음
					console.log('통신 성공!'+result);
					alert('회원 가입을 환영합니다!');
					location.href='/'; //home.jsp로 가라는 의미
     ...
     //ID 입력값 검증(공백, 정규표현식)
      ...
      //로그인 버튼 클릭 이벤트  ( ID,비밀번호 둘다 올바른 값이어야 이벤트 진행)
      //chk1,chk2를 재활용해서 사용했습니다.
      //ajax를 이용한 비동기 방식으로 로그인을 처리할 예정입니다
      //비동기 처리를 안해도 되지만 ,연습을 위해서 버디오 처리를 합니다.
      //굳이 비동기 처리가 필요 없는 곳은 남요하지 않아도 됩니다.
      ...
      $('#signIn-btn').click(function(){
    	  ...
    	  if(chk1 && chk2){
    		...
    		//0617 자동 로그인 체크박스가  체크가 되었는지의 여부 
    		//is() 함수는 상태 여부를 판단하여 ,논리 값을 리턴합니다.
				...
				success:function(result){
					if(result === 'idFail'){
						//console.log('아이디가 없습니다');
				         ...
				         $('#signInId').val('');
				         $('$signInId').focus(); //커서를 이동 시키고 , 스크롤도 해당 위치로 이동시키는 함수.
