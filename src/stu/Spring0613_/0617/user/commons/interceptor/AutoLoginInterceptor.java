...
public class AutoLoginInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		*/
		...
		//원하는 쿠키의 값을 한방에 꺼내오실 수 있습니다. 매가값으로(request 객체,쿠키 이름)
		Cookie loginCookie=WebUtils.getCookie(request, "loginCookie");
		
		//자동 로그인을 신청한 사람이라면 로그인 유지를 위해 세션 데이터를 만들어 줍니다.
		HttpSession session=request.getSession();
		if(loginCookie !=null) { //자동 로그인을 신청한 유저 
		...
		//true면 컨트롤러로 요청이 들어가고 ,false라면 요청을 막습니다.
		//자동 로그인 신청 여부와 상관없이 홈 화면은 무조건 봐야 하니까 조건 없이 ture를 작성합니다.
		return true;
