        ...
        <div class="container-fluid">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                           <c:forEach var="vo" items="${boardList}">
                               <tr>
                                   <td>${vo.bno}</td>
                                   <td><a href="<c:url value='/freeboard/freeDetail/${vo.bno}${pc.makeURI(pc.paging.pageNum)}' />">${vo.title}</a>
                                   &nbsp;&nbsp;&nbsp;
          ...
          <form action="<c:url value='/freeboard/freeList' />" name="pageForm">
                    ...
                    <ul id="pagination" class="pagination pagination-sm">
                       <c:if test="${pc.prev}">
                           <!-- form버전 활용 전  <li><a href="<c:url value='/freeboard/freeList${pc.makeURI(pc.beginPage-1)}' />">이전</a></li> -->
                    ...
                    <!-- 페이지 관련 버튼(이전 혹은 다음 혹은 페이지번호 )을 클릭시 같이 숨겨서 보내줄 공통 값을 선언  -->
   ...
   <script>
      ...
      /*    $('.search-btn').click(function() {
         ...
         $('#pagination').on('click','a',function(e){
        	 ...
        	 //페이지 버튼들을 감싸고 있는 form 태그를 name으로 지목하여 
        	 //그 안에 숨겨져 있는 pageNum이라는 input 태그의 value에 
        	 //위에서 얻은 data-pageNum의 값을 삽입한 후 submit 한다
