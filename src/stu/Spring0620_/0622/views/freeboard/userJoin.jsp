       ...
       $(function() {
		...
		$('#mail-check-btn').click(function(){
			...
			$.ajax({
				...
				success:function(data){
					console.log('컨트롤러가 전달한 인증 번호 : '+data);
		...
		//인증 번호 비교                      
		//blur - foucs가 벗어나는 경우 발생 (작성을 하고 나서 해당 창을 벗어나게되면 blur 발생)
		$('.mail-check-input').blur(function(){
			...
			if(inputCode ===code){
				...
				//disabled는 값이 아예 넘어가질 안아서 지향해야 함
				$('#userEmail1').attr('readonly',true);
				$('#userEmail2').attr('readonly',true); //select라서 안먹음
				$(this).css('display','none');
				
				//포커싱이 올때 , 초기값을 사용자가 선택한 값으로 무조건 설정하는 방법(select에서 readonly 동작안해서
				//이를 대신 하기 위함) 두줄 다 작성해야 함
