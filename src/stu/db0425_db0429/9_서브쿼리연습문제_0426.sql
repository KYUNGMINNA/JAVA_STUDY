...
SELECT 
 tb1.*,d.department_name FROM
(SELECT
    last_name,job_id,department_id
FROM employees
WHERE job_id='SA_MAN')tb1
JOIN departments d
...
SELECT  * FROM(
    ...
    (SELECT d.*,loc.street_address,loc.postal_code,NVL(a.result,0) AS 부서별평균급여
    FROM departments d
    JOIN locations loc
    ON d.location_id=loc.location_id
    LEFT JOIN 
    ...
    ON d.department_id=a.department_id)) b
    )
    WHERE rn>0 AND rn<=10
    ORDER BY department_id ASC;
